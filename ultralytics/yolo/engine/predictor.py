...
class BasePredictor:
    ...
    def write_results(self, idx, results, batch, verbose_path, frame_idx):
    ...
    def __call__(self, source=None, model=None, verbose_path=None, stream=False):
        """Performs inference on an image or stream."""
        self.stream = stream
        if stream:
            return self.stream_inference(source, model, verbose_path)
        else:
            return list(self.stream_inference(source, model, verbose_path))  # merge list of Result into one

    def predict_cli(self, source=None, verbose_path=None, model=None):
    ...
    def stream_inference(self, source=None, model=None, verbose_path=None):
        ...
        for batch in self.dataset:
            ...
            for i in range(n):
                ...
                if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                    s += self.write_results(i, self.results, (p, im, im0), verbose_path, frame_idx)  # 详细的推理结果写入txt中
